...
const AdminServicesection = () => {
	...
	const fetchData = async () => {
		await axios.get("/data/element/servicesection").then((res) => {
			setData((prev) => ({
				...prev,
				id: res.data[0]._id,
	...
	const handleUpload = async (e) => {
		...
		if (image) {
			...
			await axios
				.put(`/data/element/sv/${target}`, formData)
	...
	return (
		<>
			<div className="w-3/4 mx-auto flex justify-between items-center">
				<h1 className="text-4xl text-neutral-200">服務項目</h1>
			</div>
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="col-span-2">
								<div className="max-w-full mx-2 rounded-lg overflow-x-hidden flex justify-start items-center gap-1 relative">
									...
									{data.imgOne !== "" ? (
										<picture>
											<img
												src="/data/public/image/sv/imgOne.jpg"
												alt={data.name}
											/>
										</picture>
									) : (
									...
									{data.imgTwo !== "" ? (
										<picture>
											<img
												src="/data/public/image/sv/imgTwo.jpg"
												alt={data.name}
											/>
										</picture>
									) : (
									...
									{data.imgThree !== "" ? (
										<picture>
											<img
												src="/data/public/image/sv/imgThree.jpg"
												alt={data.name}
											/>
										</picture>
									) : (
									...
									{data.imgFour !== "" ? (
										<picture>
											<img
												src="/data/public/image/sv/imgFour.jpg"
												alt={data.name}
											/>
										</picture>
									) : (
							...
							<div className="col-span-1">
								...
								>
									...
									<option value="imgFour">第四張</option>
								</select>
							...
							<div className="col-span-2">
								...
								<p className="text-neutral-200">圖片尺寸：400x600</p>
							</div>
							{/* <div className="col-span-1">
								<button
									class="my-3 w-full bg-[#f3582c] hover:bg-[#bc2312]] text-white font-bold py-2 px-4 rounded-lg text-center"
...
export default AdminServicesection;
